...
{
    nh_.param<int>("/kf_tracker_2/obstacle_num", obstacle_num_, obstacle_num_); // # of maximum observable obstacle 
...
void ObstacleTrack::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& input)
{
    ...
    {   
        ...
        *cloud_filtered = cloud_2;
        ...
        // InitKalmanFilter
        initKalmanFilters(cloud_filtered);
    }
    ...
    { 
        ...
        */
        ...
        // obstacle num
        obstacle_num_ = clusterCenters.size();

        // debug msg 
        cout << "obstacle num: " << obstacle_num_ << endl;
        for (int i = 0; i < obstacle_num_; i++)
            cout << "clusterCentroids" << endl << clusterCentroids[i] << endl;
        
        KFT(input, clusterCenters);
    }
} 
...
void ObstacleTrack::initKalmanFilters(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered)
{
    // debug msg
    cout << "Before KF init and obstacle_num is " << obstacle_num_ << endl;
    KF_vec.clear();
    ...
    // debug msg
    for(int i = 0; i< obstacle_num_; i++)
        cout << i << "th clustercentroid" << endl << clusterCentroids[i] << endl;
    ...
    for(int i = 0; i < obstacle_num_; i++)
    {
        ...
        KF0.statePre.at<float>(0)=clusterCentroids[i].x;
        KF0.statePre.at<float>(1)=clusterCentroids[i].y;
        KF0.statePre.at<float>(2)=0;// initial v_x
        KF0.statePre.at<float>(3)=0;// initial v_y
        KF_vec.push_back(std::make_shared<cv::KalmanFilter>(KF0));
    }
    for (int i = 0; i < KF_vec.size(); i++)
        cout << i << "th KF:  " << KF_vec[i] <<endl;

    //debug msg
    for (int i = 0; i < KF_vec.size(); i++)
        cout<<"KF"<< i << ".satePre=" << KF_vec[i]->statePre.at<float>(0)<<","<<KF_vec[i]->statePre.at<float>(1)<<"\n";
    ...
    // debug msg
    cout << "initKalmanFilter complete" << endl;
}

void ObstacleTrack::KFT(const sensor_msgs::PointCloud2ConstPtr& input, const std::vector<geometry_msgs::Point> clusterCenters)
{
    if (KF_vec.size() != obstacle_num_)
    {   
        KF_vec.clear();
        pcl::PointCloud<pcl::PointXYZ> input_cloud;
        pcl::fromROSMsg (*input, input_cloud);
    ...
    for (int i =0; i < obstacle_num_; i++)
    {
        cv::Mat prediction = KF_vec[i]->predict();
        cv::Mat prediction_;
        prediction_ = prediction.clone();
        pred.push_back(prediction_);
        //debug msg
        cout << i << "th predict" << endl << &pred[i] << endl;
    }
    ...
    for (int i = 0; i < KFpredictions.size(); i++)
        cout << i << "th KFpredictions" << endl <<  KFpredictions[i] << endl;
    ...
    for(int clusterCount=0; clusterCount < obstacle_num_; clusterCount++)
    {
        ...
        distMat[minIndex.first]=std::vector<float>(obstacle_num_ ,10000.0);// Set the row to a high number.
    ...
    for ( int i = 0; i < objID.size(); i++)
        cout << i << "th objid"<<endl << objID[i] << endl;
    ...
    for (int i = 0; i < cc.size(); i++)
        cout << "cc is " << cc[i].at(0) << " " << cc[i].at(1) << " " << cc[i].at(2) << endl;
        
    /* The update phase */
    std::vector<cv::Mat> measMat_vec;
    std::vector<cv::Mat> estimated_vec;
    
    for (int i = 0; i < obstacle_num_; i++)
    {   
        float meas[2] = {cc[i].at(0),cc[i].at(1)};
        // debug msg
        cout << i << "th meas: " << meas[0] << " "<< meas[1] << endl;
        cv::Mat measMat = cv::Mat(2,1,CV_32F, meas);
        cv::Mat measMat_;
        measMat_ = measMat.clone();
        // debug msg
        cout<<i<<"th measMat" << endl << measMat_ << endl;
        measMat_vec.push_back(measMat_);
    }
    
    // debug msg
    cout << "measMat_vec: " << endl;
    for (int i = 0; i < measMat_vec.size(); i++)
        cout << measMat_vec[i] << endl;


    for ( int i = 0; i < obstacle_num_; i++)
    {   
        cv::Mat estimated = KF_vec[i]->correct(measMat_vec[i]);
        cv::Mat estimated_;
        estimated_ = estimated.clone();
        estimated_vec.push_back(estimated_);
    }
    ...
    for (int i = 0; i < obstacle_num_; i++)
        cout<<"[DEBUG] " << i << "th vel : ["<<(estimated_vec[i].at<float>(2)*(1e6))<<", "<<(estimated_vec[i].at<float>(3)*(1e6))<<"]"<<endl;
